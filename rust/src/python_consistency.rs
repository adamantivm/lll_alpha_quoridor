...
fn resolve_real_model_fixture_paths() -> (PathBuf, PathBuf) {
    ...
    assert!(
        pt_path.exists(),
        "Missing PT model fixture: {}. Set DEEP_QUORIDOR_PT_MODEL or add rust/fixtures/alphazero_B5W2_mv1.pt",
        pt_path.display()
    );
    assert!(
        onnx_path.exists(),
        "Missing ONNX model fixture: {}. Set DEEP_QUORIDOR_ONNX_MODEL or add rust/fixtures/alphazero_B5W2_mv1.onnx",
        onnx_path.display()
    );
